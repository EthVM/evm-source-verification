 ...
 */
export interface PullContractsServiceOptions {
  /**
...
/**
 * Encapsulates testable parts of the pull-contracts command
 */
export class PullContractsService {
  ...
  async process(
    ...
    options?: PullContractsServiceOptions,
  ): Promise<void> {
    const {
      outBodyFile,
      outBranchNameFile,
    ...
    for (const chainMatch of chains.values()) {
      for (const contractMatch of chainMatch.contracts.values()) {
        ...
        // validate the config
        validateConfig(contract, await contract.getConfig());

        // validate the input
        validateInput(contract, await contract.getInput());
